...
from parity_plot_scripts_wind_2385 import f_test, parity_plot, linreg_results, linreg_results_no_intercept
...
ax, n, r_value,slope,intercept,f,p = parity_plot(axes[0,0], plot_data, 'Bridger', plot_color = '#D2C295')
x = plot_data['Wind_MPS_mean300'].values
y = plot_data['WindSpeed'].fillna(0).values
#f, p = f_test(x, y)
print('Bridger stage ', 1,' F statistic = ',f)
...
ax, n, r_value,slope,intercept,f,p = parity_plot(axes[0,0], plot_data, 'Bridger', plot_color = '#53284f')
x = plot_data['Wind_MPS_mean300'].values
y = plot_data['WindSpeed'].fillna(0).values
#f, p = f_test(x, y)
print('Bridger (NAM12) stage ', 1,' F statistic = ',f)
print('Best fit $R^{2}$ = %0.2f \n$y$ = %0.2f$x+$%0.2f \n n = %0.2f' % (r_value**2,slope,intercept,n))
regress_add = [n, r_value**2,slope, intercept,f,p]
regress_stats.append(regress_add)
...
ax, n, r_value,slope,intercept,f,p = parity_plot(axes[0,1], plot_data, 'CarbonMapper', plot_color = '#D2C295')
x = plot_data['Wind_MPS_mean300'].values
y = plot_data['WindSpeed'].fillna(0).values
#f, p = f_test(x, y)
print('CM stage ', 1,' F statistic = ',f)
print('Best fit $R^{2}$ = %0.2f \n$y$ = %0.2f$x+$%0.2f \n n = %0.2f' % (r_value**2,slope,intercept,n))
regress_add = [n, r_value**2,slope, intercept,f,p]
regress_stats.append(regress_add)
...
ax, n, r_value,slope,intercept,f,p = parity_plot(axes[1,0], plot_data, 'GHGSat', plot_color = '#175e54')
x = plot_data['Wind_MPS_mean300'].values
y = plot_data['WindSpeed'].fillna(0).values
#f, p = f_test(x, y)
print('GHGSat stage ', 1,' F statistic = ',f)
print('Best fit $R^{2}$ = %0.2f \n$y$ = %0.2f$x+$%0.2f \n n = %0.2f' % (r_value**2,slope,intercept,n))
regress_add = [n, r_value**2,slope, intercept,f,p]
regress_stats.append(regress_add)

dfRegressStats = pd.DataFrame(regress_stats, columns=cols)
plt.savefig('Allteams_parity_wind_230822_zeroing.svg', dpi = 300)
